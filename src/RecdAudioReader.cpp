...
RecdAudioReader::RecdAudioReader( const FString& sStreamReaderName )
...
RecdAudioReader::~RecdAudioReader()
{/*
  if ( m_pMbxRawItems != NULL )
  {
...
*/
enum RecdAudioReader::ReaderStatus RecdAudioReader::GetStatus( DOUBLE* pdElapsed, DOUBLE* pdTotal ) const
{
...
bool              RecdAudioReader::SetStatus( ReaderStatus eStatus )
{
  FMutexCtrl  _mtxCtrl( m_mtxReader );
/*
  if (
...
*/
BOOL	RecdAudioReader::Initial()
{ 
  return TRUE; 
}

BOOL	RecdAudioReader::Final()
{
  m_bExit = TRUE;
...
VOID	RecdAudioReader::Run()
{
  AVResult      _avResult         = eAVUndefined;
  DWORD         _dwStandbyTimeout = RecdConfig::GetInstance().GetAudioReaderStandByDelay( NULL );
...
DWORD   RecdAudioReader::GetLogMessageFlags() const
{
  DWORD _dwRetVal = 0;

  _dwRetVal |= RecdConfig::GetInstance().IsTraceExceptionEnabled  ( "Audio Reader Log Message", NULL)?FLogMessage::MT_TRACE_EXCEPTION:0;
  _dwRetVal |= RecdConfig::GetInstance().IsCatchExceptionEnabled  ( "Audio Reader Log Message", NULL)?FLogMessage::MT_CATCH_EXCEPTION:0;
  _dwRetVal |= RecdConfig::GetInstance().IsAssertionFailureEnabled( "Audio Reader Log Message", NULL)?FLogMessage::MT_ASSERTION_FAILURE:0;
  _dwRetVal |= RecdConfig::GetInstance().IsErrorInfoEnabled       ( "Audio Reader Log Message", NULL)?FLogMessage::MT_ERROR_INFO:0;
  _dwRetVal |= RecdConfig::GetInstance().IsLoggingInfoEnabled     ( "Audio Reader Log Message", NULL)?FLogMessage::MT_LOGGING_INFO:0;
  _dwRetVal |= RecdConfig::GetInstance().IsVerboseInfoEnabled     ( "Audio Reader Log Message", NULL)?FLogMessage::MT_VERBOSE_INFO:0;
  _dwRetVal |= RecdConfig::GetInstance().IsRawInfoEnabled         ( "Audio Reader Log Message", NULL)?FLogMessage::MT_RAW_INFO:0;
  _dwRetVal |= RecdConfig::GetInstance().IsEnterMethodEnabled     ( "Audio Reader Log Message", NULL)?FLogMessage::MT_ENTER_METHOD:0;
  _dwRetVal |= RecdConfig::GetInstance().IsExitMethodEnabled      ( "Audio Reader Log Message", NULL)?FLogMessage::MT_EXIT_METHOD:0;

  return _dwRetVal;
}


DWORD   RecdAudioReader::GetVerbosityLevelFlags() const
{
  DWORD _dwRetVal = 0;

  _dwRetVal |= RecdConfig::GetInstance().IsStartUpMessageEnabled       ( "Audio Reader Verbosity Level", NULL)?FLogMessage::VL_START_UP_MESSAGE:0;
  _dwRetVal |= RecdConfig::GetInstance().IsShutDownMessageEnabled      ( "Audio Reader Verbosity Level", NULL)?FLogMessage::VL_SHUT_DOWN_MESSAGE:0;
  _dwRetVal |= RecdConfig::GetInstance().IsLowPeriodicMessageEnabled   ( "Audio Reader Verbosity Level", NULL)?FLogMessage::VL_LOW_PERIODIC_MESSAGE:0;
  _dwRetVal |= RecdConfig::GetInstance().IsMediumPeriodicMessageEnabled( "Audio Reader Verbosity Level", NULL)?FLogMessage::VL_MEDIUM_PERIODIC_MESSAGE:0;
  _dwRetVal |= RecdConfig::GetInstance().IsHighPeriodicMessageEnabled  ( "Audio Reader Verbosity Level", NULL)?FLogMessage::VL_HIGH_PERIODIC_MESSAGE:0;
